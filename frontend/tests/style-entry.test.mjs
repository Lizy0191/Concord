...
test('ownership stylesheets are imported once and resolve to real source files', () => {
  const local = imports.slice(3).map(line => line.match(/^@import "(\.\/[^"\n]+)";$/)?.[1]);
  assert.ok(local.length > 0);
  assert.equal(new Set(local).size, local.length);
  for (const path of local) {
    assert.ok(path, 'Only relative ownership stylesheet imports belong after vendor CSS');
